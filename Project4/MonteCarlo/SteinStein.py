...
@jit(nopython = True)
def stein_stein(Tau, kappa, beta, rho):
    ...
    for i in range(N):
        ...
        for j in range(N):
            ...
            if j == 0:
                ...
                vol_diffusion = beta * (rho * B[j - 1] * np.sqrt(dt) + np.sqrt(1 - rho**2) * W[j - 1] * np.sqrt(dt))
                volatility_process[i][j] = volatility_process[i][j - 1] + vol_drift + vol_diffusion
                stock_process[i][j] = stock_process[i][j - 1] + volatility_process[i][j] * stock_process[i][j - 1] * B[j - 1] * np.sqrt(dt)

    return volatility_process, stock_process

@jit(nopython = True)
def stein_stein_model(kappa, beta, rho):
    ...
    for i in range(len(T)):
        ...
        stein_simulation = stein_stein(T[i], kappa, beta, rho)[1]

        for j in range(len(K)):

            win_rate = 0

            for k in range(len(stein_simulation[0])):

                if stein_simulation[k][-1] > K[j]:
                    win_rate+=1

            probability = win_rate / len(stein_simulation[0])
...
if __name__ == '__main__':
    ...
    def plot():
        ...
        vol, stock = stein_stein(1, Kappa, Beta, Rho)

        fig, (ax1, ax2) = plt.subplots(1, 2)
        fig.suptitle('Stein Stein Model')
        ...
        prices = stein_stein_model(Kappa, Beta, Rho)
